...
function QuestionReview({
  ...
  const getUserAnswerText = () => {
    if (answer.userAnswer === undefined || answer.userAnswer === null) return "Not answered";

    if (question.type === "true_false") {
      return answer.userAnswer === "true" || answer.userAnswer === true ? "True" : "False";
    }

    if (question.type === "mcq" || question.type === "multi") {
      const selectedIds = Array.isArray(answer.userAnswer) ? answer.userAnswer : [answer.userAnswer];
  ...
  const getCorrectAnswerText = () => {
    if (question.correctAnswer === undefined || question.correctAnswer === null) return "Not available";

    if (question.type === "true_false") {
      return question.correctAnswer === "true" || question.correctAnswer === true ? "True" : "False";
    }
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6 mb-4">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <h4 className="font-semibold text-gray-900 text-sm mb-1">{String(question.text)}</h4>
        </div>
        <div
      ...
      {!answer.isCorrect && (
        <div className="mb-4">
          <p className="text-xs text-gray-600 font-medium mb-2">Correct Answer:</p>
    ...
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm mb-6">
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div>
            <h3 className="text-lg font-bold text-gray-900">{result.quizTitle}</h3>
            {result.courseName && <p className="text-sm text-gray-600">{result.courseName}</p>}
            <p className="text-xs text-gray-500 mt-2">{formattedDate}</p>
          </div>
          <div
            className={`text-right py-2 px-4 rounded-lg font-bold ${
              result.isPassed ? "bg-green-500 text-white" : "bg-red-500 text-white"
            }`}
          >
          ...
          result.questions.map((q, idx) => {
            ...
            return (
              <div key={q.id} className="mb-4">
                <p className="text-xs text-gray-500 font-semibold mb-2">Question {idx + 1}</p>
                {answer ? <QuestionReview question={q} answer={answer} /> : <p className="text-gray-600 text-sm">No answer submitted</p>}
              </div>
            );
          })
        ) : (
    ...
    averageScore:
      results.length > 0
        ? (
            results.reduce(
              (sum, r) =>
                sum + (r.totalScore && r.maxScore ? (r.totalScore / r.maxScore) * 100 : 0),
              0
            ) / results.length
          ).toFixed(1)
        : "0",
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Review your quiz attempts and track your progress</p>
        </div>
        ...
        {results.length > 0 ? (
                  ...
                  {(["all", "passed", "failed"] as const).map((filter) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors whitespace-nowrap ${
                        filterBy === filter ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                      }`}
                    >
          ...
          <div className="bg-gray-50 rounded-lg border border-gray-200 p-12 text-center">
            <h3 className="text-gray-900 font-bold text-lg mb-2">No Quiz Results Yet</h3>
            <p className="text-gray-600 mb-6">Complete some quizzes to see your results and track your progress!</p>
            <button
