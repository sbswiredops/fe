  ...
  return (
    <>
      ...
      <div>
        ...
        <input type="file" name="avatar" accept="image/*,video/*" onChange={onChange} className="block w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-[#51356e] hover:file:bg-blue-100" />
        {formData.avatar instanceof File && (<p className="mt-1 text-xs text-gray-500">Selected: {formData.avatar.name}</p>)}
      </div>
      <div>
      ...
      <div className="flex items-center gap-2 pt-2">
        <input
          id="isActive"
          type="checkbox"
          checked={typeof formData.isActive === 'boolean' ? formData.isActive : (formData.status ? formData.status === 'active' : true)}
          onChange={(e) =>
