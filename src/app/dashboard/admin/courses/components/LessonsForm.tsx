        ...
        >
          ...
          {sections.map((section) => (
            <option key={section.id} value={section.id}>
              {section.title} ({section.courseName})
            </option>
          ))}
        </select>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {formData.resource instanceof File && (
            <p className="mt-1 text-xs text-gray-500">
              Selected: {formData.resource.name}
            </p>
          )}
          {typeof formData.resource === "string" && formData.resource && (
            <p className="mt-1 text-xs text-gray-500">
              Current: {formData.resource}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-6 pt-2">
        <label className="inline-flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              setFormData((p: any) => ({ ...p, isPublished: e.target.checked }))
            }
            className="h-4 w-4"
          />
        ...
        <label className="inline-flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              setFormData((p: any) => ({ ...p, isFree: e.target.checked }))
            }
            className="h-4 w-4"
          />
