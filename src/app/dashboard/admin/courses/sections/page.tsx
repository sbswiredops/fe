...
function SectionsManagement() {
  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      try {
        const anyRes: any = await CourseService.getCourses({
          page: 1,
          limit: 100,
        });
        const courseArr: any[] = Array.isArray(anyRes?.data)
          ? anyRes.data
          : Array.isArray(anyRes?.data?.courses)
          ? anyRes.data.courses
          : Array.isArray(anyRes?.courses)
          ? anyRes.courses
          : [];
  ...
  React.useEffect(() => {
    const loadSections = async () => {
      ...
      for (const c of courses as any[]) {
        try {
          const res: any = await sectionService.listByCourse(c.id, {
            page: 1,
            limit: 500,
          });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setSections((prev) =>
          prev.filter((s: any) => s.id !== selectedItem.id)
        );
        setRefreshTick((x) => x + 1);
      } else {
